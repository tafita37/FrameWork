...
extends HttpServlet {
    ...
    throws Exception {
        Object[] keys=util.getView().getData().keySet().toArray();
        for(int i=0; i<keys.length; i++) {
            request.setAttribute((String) keys[i], util.getView().getData().get(keys[i]));
        }
    ...
    throws IOException, ServletException {
        ...
        try {
            ...
            RequestDispatcher dispat=request.getRequestDispatcher(util.getView().getView());
